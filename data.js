...
// PARTICIPLE REFERENCE TABLES — one per day that introduces new past
// participles in the passé composé unit (days 21, 22, 24, 25, 27). Each is a
// simple infinitive → participle lookup, styled like impTu/impNousVous
// above (arrow rows, not pronoun conjugation), so a beginner can see exactly
// which verb each participle came from, right next to the avoir/être helper
// table for that day. Kept separate from CONJUGATIONS' pronoun tables on
// purpose — these answer "what is this word doing here?", not "how do I
// conjugate this verb?".
const PARTICIPLES = {
  jour21: {
    title: "D'où viennent ces participes ? (verbes en -er)",
    rows: [["parler →", "parlé"], ["manger →", "mangé"], ["regarder →", "regardé"], ["aimer →", "aimé"], ["écouter →", "écouté"], ["travailler →", "travaillé"], ["habiter →", "habité"]],
    note: "Regular -er verbs are predictable: drop -er, add -é. This is why they don't need a separate lookup table like the irregular ones on Day 22 do."
  },
  jour22: {
    title: "D'où viennent ces participes ? (irréguliers)",
    rows: [["faire →", "fait"], ["avoir →", "eu"], ["prendre →", "pris"], ["mettre →", "mis"], ["dire →", "dit"], ["voir →", "vu"]],
    note: "Each participle comes from its own verb, not from a pattern. The helper (j'ai, tu as, il a...) is always avoir conjugated — even when avoir is the main verb, its own participle (eu) still goes at the end. You can't skip the participle, and you can't double avoir (\"j'ai avoir\" doesn't exist)."
  },
  jour24: {
    title: "D'où viennent ces participes ?",
    rows: [["aller →", "allé(e)"], ["venir →", "venu(e)"]],
    note: "These two use être as the helper instead of avoir, because they describe movement. The participle also changes spelling to match the subject: il est allé, elle est allée."
  },
  jour25: {
    title: "D'où viennent ces participes ?",
    rows: [["arriver →", "arrivé(e)"], ["partir →", "parti(e)"]],
    note: "Same être + agreement pattern as aller/venir on Day 24 — the participle's ending changes with the subject's gender."
  },
  jour27: {
    title: "D'où viennent ces participes ?",
    rows: [["rester →", "resté(e)"], ["sortir →", "sorti(e)"]],
    note: "Still être-verbs, still agreement: il est resté, elle est restée, ils sont sortis, elles sont sorties."
  }
};
